...
double MLMinBranchLength = 1.0; //e-10;         //5.0e-4; /* minimum value for branch length */
...
double MLMinBranchLength = 1.0; //e-10; //5.0e-9;
...
int main(int argc, char **argv)
{
  ...
  for (iArg = 1; iArg < argc; iArg++)
  {
    ...
    else if (strcmp(argv[iArg], "-blmin") == 0  && iArg < argc - 1) {
      iArg++;
      double blmin = atof(argv[iArg]);
  ...
  { /* Report settings */
    ...
    if (tophitsClose > 0)
      sprintf(tophitsCloseStr, "%.2f", tophitsClose);
    if (tophitsMult > 0)
    ...
    for (i = 0; i < nFPs; i++)
    {
      ...
      fprintf(fp, "\n%s distances: %s Joins: %s Support: %s\n",
              ...
              matrixPrefix ? matrixPrefix : (useMatrix ? "BLOSUM45" : (nCodes == 4 && logdist ? "Jukes-Cantor" : "%different")),
      ...
      if (MLnni != 0 || MLlen)
      {
        fprintf(fp, "ML Model: %s,",
                (nCodes == 4) ? (bUseGtr ? "Generalized Time-Reversible" : "Jukes-Cantor") : (transitionFile ? transitionFile : (bUseLg ? "Le-Gascuel 2008" : (bUseWag ? "Whelan-And-Goldman" : "Jones-Taylor-Thorton"))));
        if (nRateCats == 1)
  ...
  for (iAln = 0; iAln < nAlign; iAln++)
  {
    ...
    else
    {
      ...
      if (nCodes == 20)
      {
        transmat = transitionFile ? ReadAATransitionMatrix(transitionFile) : (bUseLg ? CreateTransitionMatrix(matrixLG08, statLG08) : (bUseWag ? CreateTransitionMatrix(matrixWAG01, statWAG01) : CreateTransitionMatrix(matrixJTT92, statJTT92)));
      }
      ...
      NJ_t *NJ = InitNJ(unique->uniqueSeq, unique->nUnique, aln->nPos,
                        ...
                        transmat);
      if (verbose > 2)
        fprintf(stderr, "read %s seqs %d (%d unique) positions %d nameLast %s seqLast %s\n",
	...
	*/
      ...
      for (i = 0; i < nFPs; i++)
      {
        ...
        if (verbose > 1 || fp == fpLog)
        {
          ...
          if (!slow)
            fprintf(fp, " Hill-climb: %ld Update-best: %ld\n", nHillBetter, nVisibleUpdate);
        ...
        fprintf(fp, "profile-freq-alloc %ld avoided %.2f%%\n",
                svProfileFreqAlloc,
                svProfileFreqAvoid > 0 ? 100.0 * svProfileFreqAvoid / (double)(svProfileFreqAlloc + svProfileFreqAvoid)
                                       : 0);
#endif
        fflush(fp);
      }
             ...
             /*OPTIONAL*/ transition_matrix_t *transmat)
{
  ...
  double fACGTUN = (counts['A'] + counts['C'] + counts['G'] + counts['T'] + counts['U'] + counts['N'] + counts['a'] + counts['c'] + counts['g'] + counts['t'] + counts['u'] + counts['n']) / (double)(totCount - counts['-'] - counts['.']);
  ...
  if (verbose > 10)
    fprintf(stderr, "Made sequence profiles\n");
  ...
  NJ->outprofile = OutProfile(NJ->profiles, NJ->nSeq,
                              ...
                              NJ->distance_matrix);
  if (verbose > 10)
    fprintf(stderr, "Made out-profile\n");
...
void FastNJ(NJ_t *NJ)
{
  ...
  if (tophitsMult > 0)
  {
    ...
    if (m < 4 || 2 * m >= NJ->nSeq)
    {
      m = 0;
      if (verbose > 1)
        fprintf(stderr, "Too few leaves, turning off top-hits\n");
    }
    else
    {
      if (verbose > 2)
        fprintf(stderr, "Top-hit-list size = %d of %d\n", m, NJ->nSeq);
    }
  }
  assert(!(slow && m > 0));
  ...
  for (nActive = NJ->nSeq; nActive > 3; nActive--)
  {
    ...
    if (bionj && join.weight > 0.01 && varIJ > 0.001)
    {
      ...
      */
      ...
      if (bionjWeight > 1)
        bionjWeight = 1;
      if (verbose > 2)
        fprintf(stderr, "dVarO %f dVarDiam %f varIJ %f from dist %f weight %f (pos %d) bionjWeight %f %f\n",
    ...
    if (verbose > 2)
      fprintf(stderr, "Join\t%d\t%d\t%.6f\tlambda\t%.6f\tselfw\t%.3f\t%.3f\tnew\t%d\n",
    ...
    NJ->diameter[newnode] = bionjWeight * (NJ->branchlength[join.i] + NJ->diameter[join.i]) + (1 - bionjWeight) * (NJ->branchlength[join.j] + NJ->diameter[join.j]);
    NJ->varDiameter[newnode] = bionjWeight * NJ->varDiameter[join.i] + (1 - bionjWeight) * NJ->varDiameter[join.j] + bionjWeight * (1 - bionjWeight) * varIJ;
    ...
    if (changedActiveOutProfile >= nResetOutProfile && changedActiveOutProfile >= fResetOutProfile * nActiveOutProfileReset)
    {
      ...
      if (verbose > 2)
        fprintf(stderr, "Recomputing outprofile %d %d\n", nActiveOutProfileReset, nActive - 1);
      ...
      if (visible != NULL)
      {
        ...
        if (besthitNew != NULL)
        {
          ...
          for (iNode = 0; iNode < NJ->maxnode; iNode++)
          {
            ...
            {
              if (verbose > 3)
                fprintf(stderr, "Visible %d reset from %d to %d (%f vs. %f)\n",
                        iNode, iOldVisible,
  ...
  if (visible != NULL)
    visible = myfree(visible, sizeof(besthit_t) * NJ->maxnodes);
  if (besthitNew != NULL)
    besthitNew = myfree(besthitNew, sizeof(besthit_t) * NJ->maxnodes);
...
void FastNJSearch(NJ_t *NJ, int nActive, /*IN/OUT*/ besthit_t *besthits, /*OUT*/ besthit_t *join)
{
  ...
  for (iNode = 0; iNode < NJ->maxnode; iNode++)
  {
    ...
    if (NJ->parent[iNode] < 0 && NJ->parent[jNode] < 0)
    { /* both i and j still active */
...
void PrintNJInternal(FILE *fp, NJ_t *NJ, bool useLen)
{
  ...
  while (stackSize > 0)
  {
    ...
    if (node < NJ->nSeq)
    {
      if (NJ->child[NJ->parent[node]].child[0] != node)
        fputs(",", fp);
      fprintf(fp, "%d", node);
      if (useLen)
    ...
    {
      if (node != NJ->root && NJ->child[NJ->parent[node]].child[0] != node)
        fprintf(fp, ",");
      fprintf(fp, "(");
      stackSize++;
  ...
  else
  {
    ...
    while (fgets(buf, sizeof(buf), fp))
    {
      ...
      else
      {
        ...
        if (verbose > 10)
          fprintf(stderr, "Read iSeq %d name %s seqsofar %s\n", iSeq, names[iSeq], seqs[iSeq]);
        iSeq++;
                        ...
                        unsigned long counts[256])
{
  ...
  for (i = 0; i < nPos; i++)
  {
    ...
    if (verbose > 10 && i < 2)
      fprintf(stderr, "pos %d char %c code %d\n", i, seq[i], c);
                            ...
                            /*OUT*/ double *distances)
{
  ...
  if (pseudoWeight > 0)
  {
    ...
    for (iHit = 0; iHit < (nProfiles * (nProfiles - 1)) / 2; iHit++)
      distances[iHit] = (distances[iHit] * hit[iHit].weight + prior * pseudoWeight) / (hit[iHit].weight + pseudoWeight);
  }
  if (logdist)
  {
    for (iHit = 0; iHit < (nProfiles * (nProfiles - 1)) / 2; iHit++)
...
void QuartetConstraintPenalties(profile_t *profiles[4], int nConstraints, /*OUT*/ double penalty[3])
{
  ...
  for (iC = 0; iC < nConstraints; iC++)
  {
    ...
    if (QuartetConstraintPenaltiesPiece(profiles, iC, /*OUT*/ part))
    {
      ...
      if (verbose > 2 && (fabs(part[ABvsCD] - part[ACvsBD]) > 0.001 || fabs(part[ABvsCD] - part[ADvsBC]) > 0.001))
...
bool QuartetConstraintPenaltiesPiece(profile_t *profiles[4], int iC, /*OUT*/ double piece[3])
{
  ...
  if (nPlus >= 3 || nMinus >= 3)
    return (false);
  piece[ABvsCD] = constraintWeight * (PairConstraintDistance(nOn[0], nOff[0], nOn[1], nOff[1]) + PairConstraintDistance(nOn[2], nOff[2], nOn[3], nOff[3]));
  piece[ACvsBD] = constraintWeight * (PairConstraintDistance(nOn[0], nOff[0], nOn[2], nOff[2]) + PairConstraintDistance(nOn[1], nOff[1], nOn[3], nOff[3]));
  piece[ADvsBC] = constraintWeight * (PairConstraintDistance(nOn[0], nOff[0], nOn[3], nOff[3]) + PairConstraintDistance(nOn[2], nOff[2], nOn[1], nOff[1]));
  return (true);
}
...
int SplitConstraintPenalty(int nOn1, int nOff1, int nOn2, int nOff2)
{
  return (nOn1 + nOff2 < nOn2 + nOff1 ? (nOn1 < nOff2 ? nOn1 : nOff2)
                        ...
                        /*OPTIONAL*/ numeric_t *codeDist2)
{
  if (dmat)
  {
    ...
    else if (codeDist2 != NULL && code1 != NOCODE)
    { /* code1 vs. codeDist2 */
                          ...
                          double bionjWeight)
{
  ...
  for (i = 0; i < nPos; i++)
  {
    ...
    if (out->weights[i] > 0)
    {
      if (profile1->weights[i] > 0 && profile1->codes[i] != NOCODE && (profile2->weights[i] <= 0 || profile1->codes[i] == profile2->codes[i]))
      {
        out->codes[i] = profile1->codes[i];
      }
      else if (profile1->weights[i] <= 0 && profile2->weights[i] > 0 && profile2->codes[i] != NOCODE)
      {
                      ...
                      distance_matrix_t *dmat)
{
  ...
  for (i = 0; i < nPos; i++)
  {
    ...
    if (out->weights[i] <= 0)
      out->weights[i] = 1e-20; /* always store a vector */
    out->nVectors++;
  ...
  assert(iFreqOut == out->nVectors);
  if (verbose > 10)
    fprintf(stderr, "Average %d profiles\n", nProfiles);
  if (dmat)
                      ...
                      distance_matrix_t *dmat)
{
  ...
  for (i = 0; i < nPos; i++)
  {
    ...
    if (out->weights[i] <= 0)
      out->weights[i] = 1e-20; /* always use the vector */
...
distance_matrix_t *ReadDistanceMatrix(char *prefix)
{
  ...
  if (verbose > 1)
    fprintf(stderr, "Read distance matrix from %s\n", prefix);
                            ...
                            int nPos, int nConstraints)
{
  ...
  { /* Jukes-Cantor */
    ...
    for (i = 0; i < nPos; i++)
    {
      ...
      */
      ...
      for (j = 0; j < 4; j++)
      {
        f[j] = (f1[j] * PSame1[iRate] + (1.0 - f1[j]) * PDiff1[iRate]) * (f2[j] * PSame2[iRate] + (1.0 - f2[j]) * PDiff2[iRate]);
        lkAB += f[j];
      }
  ...
  { /* matrix model on amino acids */
    ...
    for (i = 0; i < nPos; i++)
    {
      ...
      for (j = 0; j < 20; j++)
      {
        numeric_t value = vector_dot_product_rot(fMult1, fMult2, &transmat->codeFreq[j][0], 20) * transmat->statinv[j];
    ...
    for (i = 0; i < nPos; i++)
    {
      ...
      if (!(lkAB > 0))
      {
	   ...
	   so report all the details */
        ...
        fprintf(stderr, "fA = c(");
        for (j = 0; j < nCodes; j++)
          fprintf(stderr, "%s %.8g", j == 0 ? "" : ",", fA[j]);
        fprintf(stderr, ")\n");

        fprintf(stderr, "fB = c(");
        for (j = 0; j < nCodes; j++)
          fprintf(stderr, "%s %.8g", j == 0 ? "" : ",", fB[j]);
        fprintf(stderr, ")\n");

        fprintf(stderr, "stat = c(");
        for (j = 0; j < nCodes; j++)
          fprintf(stderr, "%s %.8g", j == 0 ? "" : ",", transmat->stat[j]);
        fprintf(stderr, ")\n");

        fprintf(stderr, "eigenval = c(");
        for (j = 0; j < nCodes; j++)
          fprintf(stderr, "%s %.8g", j == 0 ? "" : ",", transmat->eigenval[j]);
        fprintf(stderr, ")\n");

        fprintf(stderr, "expeigen = c(");
        for (j = 0; j < nCodes; j++)
          fprintf(stderr, "%s %.8g", j == 0 ? "" : ",", expeigen[j]);
        fprintf(stderr, ")\n");
                         ...
                         /*OPTIONAL OUT*/ double *site_likelihoods)
{
  ...
  double quartetloglk = loglkABCvsD + PairLogLk(pAB, pC, branch_lengths[LEN_I] + branch_lengths[LEN_C], qopt.nPos, qopt.transmat, qopt.rates,
...
#pragma omp section
    ...
    if (mlAccuracy < 2)
    {
      /* If clearly worse then ABvsCD, or have short internal branch length and worse, then
         give up */
      if (criteria[ACvsBD] < criteria[ABvsCD] - closeLogLkLimit || (lenACvsBD[LEN_I] <= 2.0 * MLMinBranchLength && criteria[ACvsBD] < criteria[ABvsCD]))
        bConsiderAC = false;
      if (criteria[ADvsBC] < criteria[ABvsCD] - closeLogLkLimit || (lenADvsBC[LEN_I] <= 2.0 * MLMinBranchLength && criteria[ADvsBC] < criteria[ABvsCD]))
        bConsiderAD = false;
      ...
      if (criteria[ACvsBD] > criteria[ABvsCD] + closeLogLkLimit && criteria[ACvsBD] > criteria[ADvsBC] + closeLogLkLimit)
        break;
      if (criteria[ADvsBC] > criteria[ABvsCD] + closeLogLkLimit && criteria[ADvsBC] > criteria[ACvsBD] + closeLogLkLimit)
        break;
    }
...
void SetMLRates(/*IN/OUT*/ NJ_t *NJ, int nRateCategories)
{
  ...
  */
  ...
  for (iPos = 0; iPos < NJ->nPos; iPos++)
  {
    ...
    for (iRate = 0; iRate < nRateCategories; iRate++)
    {
      double site_loglk_with_prior = site_loglk[NJ->nPos * iRate + iPos] + 2.0 * log(rates[iRate]) - 3.0 * rates[iRate];
...
double GammaLogLk(/*IN*/ siteratelk_t *s, /*OPTIONAL OUT*/ double *gamma_loglk_sites)
{
  ...
  for (iRate = 0; iRate < s->nRateCats; iRate++)
  {
    ...
    double pMax = iRate == s->nRateCats - 1 ? 1.0 : PGamma(s->mult * (s->rates[iRate] + s->rates[iRate + 1]) / 2.0, s->alpha);
        ...
        /*OUT*/ double *dMaxDelta)
{
  ...
  *dMaxDelta = 0.0;
  ...
  if (fastNNI)
  {
    for (node = 0; node < NJ->maxnode; node++)
    {
      if (node != NJ->root && node >= NJ->nSeq && stats[node].age >= 2 && stats[node].subtreeAge >= 2 && stats[node].support > supportThreshold)
      {
        int nodeABCD[4];
  ...
  while ((node = TraversePostorder(node, NJ, /*IN/OUT*/ traversal, /*pUp*/ NULL)) >= 0)
  {
    ...
    if (bBadConstr && verbose > 2)
    {
      ...
      for (iC = 0; iC < NJ->nConstraints; iC++)
      {
        ...
        if (QuartetConstraintPenaltiesPiece(profiles, iC, /*OUT*/ ppart))
        {
          if (ppart[qAB] + ppart[qCD] > ppart[qAD] + ppart[qBC] + tolerance || ppart[qAB] + ppart[qCD] > ppart[qAC] + ppart[qBD] + tolerance)
            fprintf(stderr, " %d (%d/%d %d/%d %d/%d %d/%d)", iC,
                    profiles[0]->nOn[iC], profiles[0]->nOff[iC],
                    profiles[1]->nOn[iC], profiles[1]->nOff[iC],
                    profiles[2]->nOn[iC], profiles[2]->nOff[iC],
...
void SetDistCriterion(/*IN/OUT*/ NJ_t *NJ, int nActive, /*IN/OUT*/ besthit_t *hit)
{
  ...
  hit->dist += constraintWeight * (double)JoinConstraintPenalty(NJ, hit->i, hit->j);
  SetCriterion(NJ, nActive, /*IN/OUT*/ hit);
}

void SetCriterion(/*IN/UPDATE*/ NJ_t *NJ, int nActive, /*IN/OUT*/ besthit_t *join)
{
  if (join->i < 0 || join->j < 0 || NJ->parent[join->i] >= 0 || NJ->parent[join->j] >= 0)
    return;
  assert(NJ->nOutDistActive[join->i] >= nActive);
  assert(NJ->nOutDistActive[join->j] >= nActive);
...
void SetOutDistance(NJ_t *NJ, int iNode, int nActive)
{
  ...
  */
  double top = (nActive - 1) * (dist.dist * dist.weight * nActive - NJ->selfweight[iNode] * NJ->selfdist[iNode]);
  double bottom = (dist.weight * nActive - NJ->selfweight[iNode]);
  double pdistOutWithoutA = top / bottom;
  NJ->outDistances[iNode] = bottom > 0.01 ? pdistOutWithoutA - NJ->diameter[iNode] * (nActive - 1) - (NJ->totdiam - NJ->diameter[iNode])
                                          : 3.0;
...
#pragma omp critical
    ...
    {
      if (verbose > 2)
        fprintf(stderr, "Skipping seed %d\n", seed);
      continue;
    }
    ...
    */
    ...
    if (verbose > 2)
      fprintf(stderr, "Distance limit for close neighbors %f weight %f ungapped %d\n",
    ...
    for (iClose = 0; iClose < tophits->m; iClose++)
    {
      ...
      bool close = closehit->dist <= neardist && (closehit->weight >= nearweight || closehit->weight >= (NJ->nPos - nGaps[closeNode]) * nearcover);
      bool identical = closehit->dist < 1e-6 && fabs(closehit->weight - (NJ->nPos - nGaps[seed])) < 1e-5 && fabs(closehit->weight - (NJ->nPos - nGaps[closeNode])) < 1e-5;
      ...
      else if (close || identical || (fastest && iClose < (tophits->q + 1) / 2))
      {
        ...
        nCloseUsed++;
        if (verbose > 2)
          fprintf(stderr, "Near neighbor %d (rank %d weight %f ungapped %d %d)\n",
  ...
  if (verbose >= 2)
    fprintf(stderr, "#Close neighbors among leaves: 1st-level %ld 2nd-level %ld seeds %ld\n",
                ...
                /*IN/OUT*/ top_hits_t *tophits)
{
  ...
  */
  ...
  bool bUseUnique = nUnique == nActive - 1 || (lNew->age <= tophitAgeLimit && nUnique >= (bSecondLevel ? (int)(0.5 + tophits2Refresh * tophits->q)
  ...
  if (!bUseUnique && bSecondLevel && lNew->age <= tophitAgeLimit)
  {
    ...
    */
    if (source != newnode && source >= 0 && tophits->top_hits_lists[source].hitSource < 0)
    {
  ...
  else
  {
    ...
    if (verbose > 2)
      fprintf(stderr, "Top hits for %d by refresh (%d unique age %d) nActive=%d\n",
                   ...
                   /*IN/OUT*/ top_hits_t *tophits)
{
  ...
  for (iHit = 0; iHit < nTopHits; iHit++)
  {
    ...
    if (hit->i < 0)
      continue; /* possible empty entries */
    ...
    if (!bSuccess || hit->criterion < visible.criterion)
    {
      ...
      if (verbose > 5)
        fprintf(stderr, "NewVisible %d %d %f\n",
                hit->j, v->j, v->dist);
    }
  } /* end loop over hits */
}
                    ...
                    /*OUT*/ besthit_t *join)
{
  ...
  if (2 * tophits->topvisibleAge > tophits->m || (3 * nCandidate < tophits->nTopVisible && 3 * nCandidate < nActive))
  {
    ...
    if (tophits->topvisibleAge <= 2)
    {
      ...
      for (iNode = 0; iNode < NJ->maxnode; iNode++)
      {
        ...
        if (newj >= 0 && newj != v->j)
        {
          ...
          assert(newj >= 0 && newj < NJ->maxnodes && newj != iNode && NJ->parent[newj] < 0);
             ...
             double *foptx, double *f2optx, double fax, double fbx, double fcx)
{
  ...
  for (iter = 1; iter <= ITMAX; iter++)
  {
    ...
    tol1 = ftol * fabs(x);
    tol2 = 2.0 * (tol1 + ZEPS);
    if (fabs(x - xm) <= (tol2 - 0.5 * (b - a)) || fabs(a - b) < atol)
    {
      *foptx = fx;
      xw = x - w;
      wv = w - v;
...
*/
  ...
  z = 1 / (x * x);
  return f + (x - 0.5) * (double)log(x) - x + .918938533204673 + (((-.000595238095238 * z + .000793650793651) * z - .002777777777778) * z + .083333333333333) / x;
}
...
hashiterator_t FindMatch(hashstrings_t *hash, char *string)
{
  ...
  while (hash->buckets[hi].string != NULL && strcmp(hash->buckets[hi].string, string) != 0)
  {
    hi++;
    if (hi >= hash->nBuckets)
      hi = 0;
...
double pythag(double a, double b)
{
  ...
  return (absa > absb) ? absa * sqrt(1 + (absb / absa) * (absb / absa)) : absb == 0 ? 0
